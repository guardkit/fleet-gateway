"""pytest-bdd collection bridge for ``features/`` (FEAT-FG-001 BDD oracle).

Two responsibilities — both required to make GuardKit's
``bdd_runner.run_bdd_for_task`` succeed when it points ``pytest`` at a
literal ``.feature`` path:
...
Pattern adapted from the canonical study-tutor implementation in
``study-tutor/features/conftest.py``.
"""
...
def _glue_candidates(feature_path: Path) -> List[Path]:
    """Per-task glue lookup (TASK-AB-004), with legacy fallback.

    Priority:

    1. ``test_<slug>__<sanitised_task_id>.py`` when ``GUARDKIT_BDD_TASK_ID``
       is set in the env (parallel-safe per-task glue).
    2. ``test_<slug>.py`` (legacy shared glue, single-task projects).

    The per-task entry is omitted when the env var is unset so local
    pytest invocations behave exactly as before.
    """
...
class _FeatureFile(pytest.File):
    ...
    Per-task glue lookup (TASK-AB-004): when ``GUARDKIT_BDD_TASK_ID`` is
    set in the env, ``test_<slug>__<sanitised_task_id>.py`` is preferred
    over the legacy shared ``test_<slug>.py``. Solves the FEAT-FG-001
    race where two parallel tasks rewrote the same shared module.
    """
